...
public class LogisticRegressionController {
	...
	@Autowired LogisticRegressionService logisticRegressionService;

	@RequestMapping("/logisticregression")
	public @ResponseBody Map<String, Object> fileUploadHandler(HttpServletRequest request, LinearRegressionInput input) {
		...
		Model res = logisticRegressionService.analysis(input.getValue(), input.getColumn());
	...
	@RequestMapping("/check/logisticregression")
	...
	@RequestMapping("/destory/logisticregression")
	...
	@RequestMapping("/predict/logisticregression")
	public @ResponseBody Map<String, Object> predictHandler(HttpServletRequest request, LinearRegressionInput input) {
		...
		String res = logisticRegressionService.predict(input.getValue(), model);
		if (res.length() == 0) {
			map.put("result", "fail");
		}
		else {
