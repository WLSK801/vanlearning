$(document).ready(function() {
	...
	$.ajax({
		type: "POST",
		url: "./analysis/check/decisiontree.do",
									...
									$.ajax({
										...
										success: function(data) {
											if(data.isTrained == "true") {
												$.ajax({
													type: "POST",
													url: "./analysis//predict/decisiontree.do",
													data: {
														column: 1,
					...
					if(f) {
						...
						r.onload = function(e) {
							...
							if(checkVaild(csvArray)) {
								...
								csvArray = null;
								uploadtraining_2();
								$("#output-column").attr("disabled", true);
...
function checkVaild(csvArray) {
	...
	for(var i = 1; i < csvArray.length; i++) {
		...
		for(var j = 0; j < csvArray[0].length - 1; j++) {
...
function reupload() {
	...
	$.ajax({
		type: "POST",
		url: "./analysis/destory/decisiontree.do",
		success: function(data) {
			// TODO
		}
