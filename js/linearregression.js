$(document).ready(function() {
	...
	$.ajax({
		type: "POST",
		url: "./analysis/check/linearregression.do",
									...
									$.ajax({
										...
										success: function(data) {
											if(data.isTrained == "true") {
												$.ajax({
													type: "POST",
													url: "./analysis//predict/linearregression.do",
													data: {
														column: 1,
							...
							if(checkVaild(csvArray)) {
								...
								$("#kmeans-submit").on("click", function(evt) {
									...
									if(cluNum != 'null' && !isNaN(parseInt(cluNum, 10)) &&
										...
										$.ajax({
											type: "POST",
											url: "./analysis/linearregression.do",
											data: {
												column: cluNum,
												value: JSON.stringify($.csv.toObjects(e.target.result)),
...
function checkVaild(csvArray) {
	...
	if(csvArray[0].length < 1 || csvArray[0].length >= 5) {
		return false;
	}
...
function reupload() {
	...
	$.ajax({
		type: "POST",
		url: "./analysis/destory/linearregression.do",
		success: function(data) {
			// TODO
		}
